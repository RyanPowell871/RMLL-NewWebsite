...
import { migrateDocumentsFromStorage } from "./migrate_documents.tsx";
...
app.post("/documents", async (c) => {
  try {
    ...
    // Check for duplicates by file_url to prevent re-importing same documents
    if (!body.skipDuplicateCheck) {
      const allDocuments = await db.getDocuments();
      const duplicate = allDocuments.find((doc: any) => doc.file_url === body.file_url);
...
// Update document metadata (category, subcategory, title, description, year)
...
app.delete("/documents", async (c) => {
  try {
    ...
    // Note: We're not deleting files from storage in bulk to avoid accidental data loss
    // Files in storage remain and can be cleaned up manually if needed

    console.log(`[Documents] Bulk deleted ${allDocuments.length} documents`);

    return c.json({
      success: true,
      message: `Deleted ${allDocuments.length} documents`,
      deletedCount: allDocuments.length
    });
  } catch (error) {
  ...
  try {
    ...
    for (const doc of allDocuments) {
      ...
      if (detectedYear !== null && doc.document_year !== detectedYear) {
        await db.updateDocument(doc.id, { document_year: detectedYear });
        updatedCount++;
...
// Migrate documents from storage to database
// This endpoint scans the documents storage bucket and creates database records
// for any files that don't have a corresponding record
app.post("/documents/migrate", async (c) => {
  try {
    console.log("[Documents] Starting document migration...");

    const result = await migrateDocumentsFromStorage();
    ...
    console.error("[Documents] Error in migration endpoint:", error);
    return c.json({ success: false, error: error.message }, 500);
  }
});
